...
int main (int argc, char *argv[])
{
    ...
    for (int i = 1; ; i++) {
        ...
        while(1) {
            ...
            up_bound_seqnum = next_expected + ((WND_SIZE - 1) * PKT_SIZE);
            low_bound_seqnum = next_expected - (WND_SIZE * PKT_SIZE);
            
            if (n > 0) {
                printRecv(&recvpkt);

                if (recvpkt.fin) {
                    cliSeqNum = (next_expected + 1) % MAX_SEQN;
                ...
                else if(recvpkt.seqnum == next_expected){
                    //write to the file
                    fwrite(recvpkt.payload, 1, recvpkt.length, fp);
                    next_expected = (recvpkt.seqnum + recvpkt.length) % MAX_SEQN;
                    buildPkt(&ackpkt, ackpkt.seqnum, next_expected, 0, 0, 1, 0, 0, NULL);
                    printSend(&ackpkt, 0);
                    sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                }
                else{
                    if(up_bound_seqnum < MAX_SEQN && recvpkt.seqnum > next_expected){ //Larger seqnum received
                        buildPkt(&ackpkt, ackpkt.seqnum, next_expected, 0, 0, 0, 1, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    }
                    else if(up_bound_seqnum > MAX_SEQN && (recvpkt.seqnum > next_expected || (recvpkt.seqnum < up_bound_seqnum % MAX_SEQN))){
                        //If upperbound wraps around
                        buildPkt(&ackpkt, ackpkt.seqnum, next_expected, 0, 0, 0, 1, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    }
                        ...
                        buildPkt(&ackpkt, ackpkt.seqnum, dup_ack, 0, 0, 0, 1, 0, NULL);
                        printSend(&ackpkt, 0);
                        sendto(sockfd, &ackpkt, PKT_SIZE, 0, (struct sockaddr*) &cliaddr, cliaddrlen);
                    }
                    else if(low_bound_seqnum < 0 && 
                        (recvpkt.seqnum < next_expected || (recvpkt.seqnum < MAX_SEQN && recvpkt.seqnum >= MAX_SEQN + low_bound_seqnum))){
                            //Lost ACK
                            int dup_ack = (recvpkt.seqnum + recvpkt.length) % MAX_SEQN;
                            buildPkt(&ackpkt, ackpkt.seqnum, dup_ack, 0, 0, 0, 1, 0, NULL);
                            printSend(&ackpkt, 0);
